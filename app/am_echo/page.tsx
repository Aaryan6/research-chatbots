...
export default async function AmEchoPage() {
  ...
  if (userInfo?.value) {
    try {
      ...
      if (dbUser) {
        validatedUserInfo = {
          ...dbUser,
          group: 8 // Automatically set group for am_echo
        };
      }
    } catch (error) {
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <Hero />
        <Chat userInfo={validatedUserInfo} hideGroupSelect={true} predefinedGroup={8} />
      </div>
    </main>
  );
} 